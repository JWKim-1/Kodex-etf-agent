...
_saved_mass = st.session_state.get("mass_sheet_key")
if _saved_mass and _saved_mass in sheet_names:
    _m_idx = sheet_names.index(_saved_mass)
    if 0 <= _m_idx < len(labeled):
        _default_idx = _m_idx
...
st.session_state["mass_sheet_key"] = current_sheet
...
기준 컬럼: **개인 순매수** (금융투자·은행 제외, LP 노이즈 없음)

| 단계 | 내용 |
|------|------|
| ① 1단계 DiD | `(이번주 개인순매수 − 8주평균) ÷ (8주절댓값평균 + 라플라스α)` → KODEX 변화율 − 경쟁사평균 변화율 |
| ② 2단계 Z-score | `(이번주 DiD − 15주 DiD평균) ÷ 15주 DiD표준편차` |
| ③ sigmoid 점수 | `100 ÷ (1 + exp(−Z × 1.5))` → 0~100점 |

**점수의 통계적 의미:**
경쟁사 대비 초과 개인 순매수 변화가 이번 주만큼 크게 발생할 확률이 과거 이력(15주) 분포에서 얼마나 드문가를 나타냅니다.
...
```
KODEX 비율 = 이번 주 개인순매수 / KODEX AUM(시총)
경쟁사 비율 = 이번 주 경쟁사 개인순매수 / 경쟁사 AUM
AUM DiD = KODEX비율 − 경쟁사평균비율
```

AUM으로 나누는 이유: ETF 크기가 달라도 동등하게 비교 가능 (체급 차이 제거)

- **AUM DiD > 0** → KODEX에 경쟁사 대비 상대적으로 더 많은 개인 자금 유입
- **AUM DiD < 0** → 경쟁사에 상대적으로 더 많은 개인 자금 유입
