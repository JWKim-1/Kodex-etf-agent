...
with st.expander("📐 마케팅 점수(0~100) 산정 방식", expanded=False):
    st.markdown("""
**증권 채널**은 유튜브·블로그·이벤트에서 마케팅 활동을 감지한 뒤 해당 ETF의 금융투자 순매수 변화를 이중차분법(DiD)으로 측정합니다.

| 단계 | 내용 |
|------|------|
| ① 변화율 | `(현재 금융투자순매수 − 8주평균) ÷ (8주절댓값평균 + 라플라스α)` |
| ② DiD | `KODEX 변화율 − 경쟁사평균 변화율` (시장 공통 효과 제거) |
| ③ Z-score | `(이번주 DiD − 15주 DiD평균) ÷ (15주 DiD표준편차 + 0.05)` |
| ④ sigmoid 점수 | `100 ÷ (1 + exp(−Z × 1.5))` → 0~100점 |

- **베이스라인**: 직전 8주 금융투자 순매수 평균 (라플라스 α = 1,000,000 천원 단위로 항상 더함)
- **비교군**: 가격 수익률 상관계수 ≥0.7 경쟁 ETF (TIGER·ACE·RISE·HANARO·SOL 등)
- **LP 노이즈 감지**: 금융투자 순매수에 LP 의심 시 개인 컬럼으로 자동 전환
- **기준 컬럼**: 금융투자 순매수 (LP 감지 시 개인으로 전환)

**점수의 통계적 의미:**
경쟁사 대비 초과 금융투자 순매수 변화가 이번 주만큼 크게 발생할 확률이 과거 15주 이력 분포에서 얼마나 드문가를 나타냅니다.
...
**판정 기준:** 🟢 ≥75점 효과 있음 / 🟡 ≥60점 가능성 / ⚪ ≥40점 중립 / 🔴 <40점 경쟁사 우위 · ⚠️ 비교군없음 = 절대변화율(참고용)

**베이스라인 부족 시 (신규상장 등 8주 미만) — AUM 상대강도 방식:**
이력이 부족해 Z-score 기반 점수를 산출할 수 없을 때, 이번 주 순매수를 시가총액(AUM)으로 나눈 비율로 비교합니다.

```
KODEX 비율 = 이번 주 금융투자순매수 / KODEX AUM(시총)
경쟁사 비율 = 이번 주 경쟁사 금융투자순매수 / 경쟁사 AUM
...
⚠️ Z-score 이력 없이 이번 주 단면만 측정하므로 다른 종목 점수와 직접 비교 시 주의.
    """)
...
if IS_BACKTEST and not collection_results:
    ...
    for _ev in llm_result.get("evidence", []):
        _title_raw = _ev.get("title", "") + " " + _ev.get("event_summary", "") + " " + (_ev.get("target_etf") or "")
...
# ── ETF별 상세 계산 과정 ──
if not did_results:
    st.warning("DiD 계산 결과 없음 — 분석 실행 버튼을 클릭해주세요.")
    st.stop()
