...
with _main_tab1:
    ...
    def _parse_period(period_str):
        """기간 문자열 → (start_date, end_date) 또는 None."""
        if not period_str or period_str in ("null", "None", ""):
            return None
        # 2026.01.01 ~ 2026.12.31 / 2026-01-01 ~ 2026-12-31 패턴
        m = _re.search(r"(\d{4})[.\-](\d{1,2})[.\-](\d{1,2})\s*[~\-]\s*(\d{4})[.\-](\d{1,2})[.\-](\d{1,2})", str(period_str))
        if m:
            try:
                s = _dt(int(m.group(1)), int(m.group(2)), int(m.group(3)))
                e = _dt(int(m.group(4)), int(m.group(5)), int(m.group(6)))
                return s, e
            except Exception:
                pass
        # 단일 날짜
        m2 = _re.search(r"(\d{4})[.\-](\d{1,2})[.\-](\d{1,2})", str(period_str))
        if m2:
            try:
                s = _dt(int(m2.group(1)), int(m2.group(2)), int(m2.group(3)))
                return s, s
            except Exception:
                pass
        return None
    ...
    _cal_events = []
    for ev in events:
        parsed = _parse_period(ev.get("event_period"))
        if parsed:
            s, e = parsed
            prov = ev.get("provider", "기타")
            _cal_events.append({
                ...
                "provider": prov,
                "start": s, "end": e,
                "color": COMP_PROVIDERS.get(prov, {}).get("color", "#888"),
            })
    ...
    if _cal_events:
        ...
        for i, ev in enumerate(_cal_events):
            fig_cal.add_trace(_go2.Bar(
                x=[(ev["end"] - ev["start"]).days + 1],
                y=[f"{ev['provider']}: {ev['title']}"],
                orientation="h",
                base=ev["start"].timestamp() * 1000,
                marker=dict(color=ev["color"], opacity=0.75,
                            line=dict(color="rgba(255,255,255,0.2)", width=1)),
                hovertemplate=f"<b>{ev['title']}</b><br>{ev['start'].strftime('%Y.%m.%d')} ~ {ev['end'].strftime('%Y.%m.%d')}<extra></extra>",
                showlegend=False,
            ))
        fig_cal.update_layout(
            height=max(200, len(_cal_events) * 36 + 60),
            barmode="overlay",
            paper_bgcolor="rgba(0,0,0,0)",
            plot_bgcolor="rgba(20,20,30,0.5)",
            font=dict(color="#e8eaed", size=11),
            xaxis=dict(type="date", gridcolor="rgba(255,255,255,0.07)",
                       tickformat="%m/%d", title=""),
            yaxis=dict(autorange="reversed", showgrid=False),
            margin=dict(l=10, r=20, t=10, b=30),
        )
        st.plotly_chart(fig_cal, use_container_width=True)
        st.caption(f"기간 정보 있는 이벤트 {len(_cal_events)}건 표시")
...
with _main_tab2:
    exec(open(os.path.join(_ROOT, "agents/history/app_history.py"), encoding="utf-8").read())
